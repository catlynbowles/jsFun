...
describe("PROTOTYPES", () => {
  ...
  describe("Weather prompts", () => {
    it.skip("getAverageTemps", () => {
      ...
      expect(e).to.deep.equal([40, 40, 44.5, 43.5, 57, 35, 65.5, 62, 14, 46.5]);
    }),
  ...
  describe("Brewery Prompts", () => {
    ...
    it.skip("getSingleBreweryBeerCount", () => {
      const ratioCount =
        breweryPrompts.getSingleBreweryBeerCount("Ratio Beerworks");
      const plattCount = breweryPrompts.getSingleBreweryBeerCount(
  ...
  describe("Board Game Prompts", () => {
    it.skip("listGames", () => {
      ...
      expect(strategyGames).to.deep.equal([
        "Chess",
        "Catan",
        "Checkers",
        "Pandemic",
      ...
      expect(childrensGames).to.deep.equal([
        "Candy Land",
        "Connect Four",
        "Operation",
        "Trouble",
      ]);
      expect(partyGames).to.deep.equal([
        "Werewolf",
    ...
    it.skip("listGamesAlphabetically", () => {
      const strategyGames =
        boardGamePrompts.listGamesAlphabetically("strategy");
      const childrensGames =
        boardGamePrompts.listGamesAlphabetically("childrens");
    ...
    it.skip("findHighestRatedGamesByType", () => {
      const highestStrategy =
        boardGamePrompts.findHighestRatedGamesByType("strategy");
      const highestChildrens =
        boardGamePrompts.findHighestRatedGamesByType("childrens");
      const highestParty =
        boardGamePrompts.findHighestRatedGamesByType("party");

      expect(highestStrategy).to.deep.equal({
        name: "Azul",
        rating: 8.8,
        maxPlayers: 4,
      });
      expect(highestChildrens).to.deep.equal({
        name: "Connect Four",
        rating: 4.9,
        maxPlayers: 2,
      });
      expect(highestParty).to.deep.equal({
        name: "Codenames",
        rating: 7.4,
        maxPlayers: 8,
      });
    ...
    it.skip("averageScoreByTypeAndPlayers", () => {
      const avScoreStrat = boardGamePrompts.averageScoreByTypeAndPlayers(
        "strategy",
        2
      );
      const avScoreChildren = boardGamePrompts.averageScoreByTypeAndPlayers(
        "childrens",
        4
      );
      const avScoreParty = boardGamePrompts.averageScoreByTypeAndPlayers(
        "party",
        8
      );
